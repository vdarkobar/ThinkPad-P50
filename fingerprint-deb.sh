...
ensure_initializer_runtime_tools() {
  log "Checking initializer runtime tools"

  local missing=()
  local cmd
  for cmd in innoextract 7z; do
  ...
  if (( ${#missing[@]} > 0 )); then
    die "Missing initializer runtime tool(s): ${missing[*]}. Re-run package installation or install the missing Debian package(s)."
  fi
}
