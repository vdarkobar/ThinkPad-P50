...
#
# One-liner (replace <RAW_URL> with the raw link to this file on your platform):
...
for group_name in libvirt kvm; do
    if getent group "$group_name" >/dev/null 2>&1; then
        if id -nG "$USER" | tr ' ' '\n' | awk -v group="$group_name" '$0 == group { found = 1 } END { exit found ? 0 : 1 }'; then
            success "$USER is already in the $group_name group"
        else
            sudo usermod -aG "$group_name" "$USER"
            warn "Added $USER to the $group_name group — log out/in or reboot before using virt-manager without sudo"
        fi
    else
...
if systemd_unit_exists libvirtd.service; then
    if sudo systemctl enable --now libvirtd.service; then
        ...
        warn "Could not enable/start libvirtd.service"
    fi
else
    LIBVIRT_MODULAR_UNIT_FOUND=0

    for unit in virtqemud.service virtnetworkd.service virtstoraged.service; do
        if systemd_unit_exists "$unit"; then
            LIBVIRT_MODULAR_UNIT_FOUND=1
    ...
    if [[ "$LIBVIRT_MODULAR_UNIT_FOUND" == "0" ]]; then
        warn "No libvirtd or modular libvirt systemd units found"
    fi
fi
...
printf '  • virt-manager/libvirt installed; log out/in or reboot for libvirt/kvm group membership.\n'
