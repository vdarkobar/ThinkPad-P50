...
if [[ -z "${DBUS_SESSION_BUS_ADDRESS:-}" ]]; then
    ...
    if gsettings list-schemas 2>/dev/null | grep -q '^org.gnome.Terminal.ProfilesList$'; then
        ...
        if [[ -n "$TERM_PROFILE" ]]; then
            ...
            gset_path "$TERM_SCHEMA" use-system-font false
            gset_path "$TERM_SCHEMA" font 'JetBrains Mono 12'
            gset_path "$TERM_SCHEMA" default-size-columns 120
            gset_path "$TERM_SCHEMA" default-size-rows 31
        else
            warn "Could not detect GNOME Terminal profile — skipping terminal font/size setting"
        fi
    else
        warn "GNOME Terminal schema not found — skipping terminal font/size setting"
    fi
