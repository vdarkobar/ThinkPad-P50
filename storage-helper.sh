...
TIMESHIFT_HINT_DIR="$BACKUP_MOUNT/timeshift"

FSTAB_OPTS="defaults,noatime,lazytime,nofail,x-systemd.device-timeout=10"
TEMP_FSTAB=""
...
check_label_conflict() {
    ...
    # Probe the target and bypass blkid's cache to avoid stale label results
    # immediately after mkfs/e2label. Use sudo because normal users often
    # cannot read raw block devices when the cache is bypassed.
    sudo blkid -p "$target_dev" >/dev/null 2>&1 || true
    mapfile -t existing < <(sudo blkid -c /dev/null -t "LABEL=$label" -o device 2>/dev/null || true)
...
ensure_fstab_label_mount() {
    ...
    for _ in {1..10}; do
        mapfile -t devices < <(sudo blkid -c /dev/null -t "$spec" -o device 2>/dev/null || true)
        ((${#devices[@]} > 0)) && break
        sudo udevadm settle || true
        sleep 1
    done
...
configure_backup_iso_permissions_and_pool() {
    ...
    # Timeshift can create/manage its own subdirectory. This just makes the
    # intended location obvious without taking ownership away from root.
    sudo install -d -o root -g root -m 0755 "$TIMESHIFT_HINT_DIR"
...
configure_drive_role() {
    ...
    check_label_conflict "$target_dev" "$label"
    ensure_fstab_label_mount "$label" "$mountpoint" ext4 "$FSTAB_OPTS"
