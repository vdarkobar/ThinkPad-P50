...
check_label_conflict() {
    ...
    # immediately after mkfs/e2label.
    blkid -p "$target_dev" >/dev/null 2>&1 || true
    mapfile -t existing < <(blkid -c /dev/null -t "LABEL=$label" -o device 2>/dev/null || true)
...
use_existing_ext4_for_role() {
    ...
    current_label="$(blkid -s LABEL -o value "$target_dev" 2>/dev/null || true)"
...
ensure_fstab_label_mount() {
    ...
    mapfile -t devices < <(blkid -c /dev/null -t "$spec" -o device 2>/dev/null || true)
